...
function spawnEnemy() {
    const enemy = {
        ...
        isBoss: score % 30 === 0 && !bossSpawned && score !== 0
    };
...
function gameLoop() {
    ...
    if (!player.alive) {
        return;  // If player is not alive, exit the game loop to avoid multiple alerts
    }
    ...
    enemies.forEach((enemy, index) => {
        ...
        if (enemy.isBoss) {
			...
			if (Date.now() > lastBossMoveChange + bossMoveInterval) {
                ...
                enemy.speed = (Math.random() * 3 - 1.5);  // -1.5에서 1.5 사이의 랜덤 속도
                lastBossMoveChange = Date.now();
            }

            enemy.x += enemy.speed;  // x축으로 움직임
            enemy.y += enemy.speed;  // y축으로 움직임
			...
			if (enemy.x <= 0) {
                enemy.x = 0;
                enemy.dir = 1;  // Change direction to right
  			}
			
  		    if (enemy.x >= canvas.width - enemy.width) {
                enemy.x = canvas.width - enemy.width;
                enemy.dir = -1;  // Change direction to left
            }

            if (enemy.y <= 0) {
                enemy.y = 0;
                enemy.speed = (Math.random() < 0.5 ? 1.5 : -1.5); // 랜덤하게 위 또는 아래로 움직임
            }
    ...
    enemyBullets.forEach((bullet, index) => {
        ...
        if (collide(player, bullet) && !gameOver) {
            player.alive = false;
